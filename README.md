...
## 리포트 양식 구성 (1페이지, 실제 증권사 데일리 스트래티지 노트 형식)

제목/부제 + 작성자 정보 박스를 맨 위에 두고, 이어서:

1. **Ⅰ. 지수 · 시장 지표** — KOSPI/KOSDAQ/원달러 환율/국제 금(현물)/WTI 원유를 표로, 각 행마다
   "왜 이렇게 움직였는지" 해석을 붙인다.
2. **Ⅱ. 업종 동향 맵** — 시가총액 비중에 정확히 비례하는 실제 면적의 업종 히트맵(트리맵). 큰
   박스에는 업종을 대표하는 종목명·시세까지 함께 표시한다.
3. **Ⅲ. 이슈 종목** — 정확히 3개. 종목명, 등락률, 움직인 이유, **지속성**(높음/중립/약세 지속),
   **언제·무엇을 확인**해야 하는지까지 종목별로 짚어준다.
4. **Ⅳ. 이번 주 캘린더** — 오늘을 포함해 요일별(월~금) 칸으로 나눈 주간 그리드. 각 칸에
   시각·이벤트·중요도(★~★★★)·체크포인트를 담고, 그 주의 핵심 하루는 칸 맨 아래에 한 줄
   요약을 붙인다. 그리드 아래에 "다음 주 예고" 한 줄.

컴플라이언스는 맨 아래 짧은 한 문단 고지로 최소화해 가독성을 우선했다.
