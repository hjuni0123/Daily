...
scripts/
  ...
  toss_client.py       # 토스증권 Open API 클라이언트 (로컬 PC에서 검증 완료)
  requirements.txt
  docx/
...
## 리포트 양식 구성 ("Daily Market Close" 1페이지 포맷)

날짜·작성자 정보 + 제목/리드 문단을 맨 위에 두고, 이어서:

1. **1. 지수 및 시장 지표** — 코스피/코스닥/원·달러 환율/WTI 원유/필라델피아 반도체를
   시가·장중고점·종가·비고 표로, 각 행마다 "왜 이렇게 움직였는지" 해석을 붙인다. 종가가
   아직 확인 안 됐으면(장마감 직후 등) 지어내지 않고 "HTS 확인"으로 남긴다. 표 아래에
   수급(외국인/기관/개인 순매수) 요약 한 줄.
2. **2. 업종 동향** — 시가총액 비중(근사)에 비례한 면적의 업종 히트맵(트리맵). 각 박스에
   업종명과 방향성(정확한 등락률 또는 "강보합"/"+1%대"/"약보합" 같은 버킷)을 표시한다.
   아래에 업종 동향을 요약하는 분석 문단.
3. **3. 이번 주 일정** — 오늘을 포함해 이번 주 안에서 실제로 의미 있는 날짜/시간대만 칸으로
   나눈 캘린더(요일마다 꼭 하나씩 만들지 않아도 되고, 같은 날이라도 새벽/오전/밤처럼
   시간대별로 쪼갤 수 있다). 각 칸에 헤드라인·부연 설명·짧은 태그(예: "개장 전 확인 필수").
4. **4. 금일 체크포인트** — 오늘 주목할 섹터 3~4개를 표로, 섹터명·관심 종목·오늘의 관점·근거를
   짚어준다.
...
python3 scripts/render_report.py reports/sample/2026-08-26_sample.json --outdir /tmp/out

# 워드(.docx) 버전도 필요하면
cd scripts/docx && npm install && cd ../..
node scripts/docx/render_docx.js reports/sample/2026-08-26_sample.json /tmp/out/2026-08-26_market_report.docx
```

`.docx`는 사용자가 제공한 워드 양식("Daily Market Close" 레이아웃)을 `docx`(npm)로
...
`reports/sample/2026-08-26_sample.json` 은 **포맷 시연용 예시 데이터**다 (실제 시황 아님).
...
1. WebSearch로 당일 코스피/코스닥 시황, 수급, 업종 동향, 금일 체크포인트, 이번 주 일정을 조사
...
### 토스증권 Open API (`scripts/fetch_toss.py`, 로컬 PC 전용)

사용자의 실제 Toss Securities Open API 키로 로컬 PC에서 검증 완료됐다 — 코스피/코스닥
지수(시가·고가·종가), 원/달러 환율, 투자자별 순매수, 등락률 상위/하위 종목을 정확하게
가져온다. 업종(sectors)·WTI·필라델피아 반도체는 Toss Open API 공식 문서에 없는
엔드포인트라 몇 가지 후보 경로/심볼을 실제로 시도해보는 EXPERIMENTAL 코드이며, 실패해도
안전하게 "-"/빈 배열로 남는다(로그에 정확한 실패 원인이 남는다).

**이 API 서버는 이 저장소가 개발되는 클라우드 샌드박스에서는 접속이 막혀 있다** — 방화벽
제약이 없는 사용자의 로컬 PC/자체 서버에서만 쓸 수 있다. 사용하려면:

1. API 키는 저장소 루트의 `.env` 파일에 넣는다 (`.env.example` 참고, 편집기로 직접 열어서
   붙여넣을 것 — 터미널에 `cat > .env <<EOF ...`처럼 치면 키가 화면/로그에 그대로 남는다).
   `.env`는 `.gitignore`에 등록되어 있어 **절대 커밋되지 않는다**.
2. 토스 콘솔에 현재 공인 IP를 허용 목록에 등록해야 한다(`curl -s https://api.ipify.org`로
   확인). **핫스팟/모바일 데이터는 IP가 수시로 바뀌어(CGNAT) 이 방식과 근본적으로 안 맞는다**
   — 유선 인터넷을 쓰거나, 매일 자동 실행이 꼭 필요하면 고정 IP가 있는 클라우드 서버(예:
   DigitalOcean)로 옮기는 걸 권장한다.
...
- 자동 생성된 데이터는 **뉴스 검색 기반 요약**이다. 배포 전 반드시 본인이 한 번
  검토(특히 수치와 체크포인트 근거)한 뒤 전파할 것을 권장한다.
